...
import { collection, query, where, getDocs, doc, updateDoc, getDoc, addDoc, orderBy, serverTimestamp, onSnapshot } from 'firebase/firestore';
...
const UserDashboard = () => {
  ...
  useEffect(() => {
    const fetchUserData = async () => {
      try {
        ...
        setBedRequests(allBedRequests.sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt)));
        ...
        if (userData) {
          ...
          setBedRequestForm(prev => ({
            ...
            patientAge: userData.dateOfBirth ? calculateAge(userData.dateOfBirth) : '',
  ...
  const handleBedRequestSubmit = async (e) => {
    ...
    if (!bedRequestForm.patientName || !bedRequestForm.requiredWard || !bedRequestForm.requiredDate) {
    ...
    try {
      const { score: priority, log: priorityLog } = calculatePriorityScore(bedRequestForm);
      ...
      // Add notification for hospital admin
      ...
      const notificationsRef = collection(db, 'hospitals', selectedHospitalForBed.id, 'notifications');
      await addDoc(notificationsRef, notificationData);
      
      // Save to Firestore
      const requestsRef = collection(db, 'hospitals', selectedHospitalForBed.id, 'bedRequests');
      ...
      setBedRequestSuccess(`Bed request sent successfully to ${selectedHospitalForBed.name}! AI Priority Score: ${priority}. The hospital will contact you soon.`);
  ...
  const handleUploadRecord = async (e) => {
    ...
    try {
      ...
      setUploadSuccess("Record uploaded securely with blockchain verification!");
  ...
  const storeHashOnBlockchain = async (patientId, recordId, hash, recordType, fileName) => {
    try {
      ...
      const tx = await contract.addRecord(patientId, recordId, hash, recordType, fileName);
      await tx.wait();
      return tx.hash;
    } catch (error) {
  ...
  const getStatusText = (status) => {
    switch(status) {
      ...
      case 'allocated': return '🛏️ Bed Allocated';
  ...
  return (
    <div className="user-dashboard">
      <div className="dashboard-header">
        <div className="header-content">
          <div style={{ display: "flex", alignItems: "center", gap: "20px" }}>
            <div onClick={handleLogoClick} style={{ cursor: "pointer" }} title="Go to Home">
              <img
                src="/images/logo.png"
          ...
          <div style={{ display: "flex", gap: "15px", alignItems: "center" }}>
            <button onClick={() => setShowSearchModal(true)} className="logout-btn" style={{ background: "rgba(255,255,255,0.2)", display: "flex", alignItems: "center", gap: "8px" }}>
              🔍 Search Hospitals
            </button>
                ...
                {searchResults.length > 0 ? (
                  searchResults.map(hospital => {
                    ...
                    beds.forEach(bed => {
                      ...
                      if (bed.status === 'available') bedsByType[bed.type].available++;
                    });
                    ...
                    return (
                      <div key={hospital.id} className="hospital-card">
                        <div className="hospital-header">
                          <h3>{hospital.name}</h3>
                          <span className={`hospital-status status-${hospital.status || 'approved'}`}>
                            {hospital.status || 'approved'}
                          </span>
                        </div>
                        <div className="hospital-details">
                          <p><strong>📍 Address:</strong> {hospital.city}, {hospital.state} - {hospital.pincode}</p>
                          <p><strong>📞 Phone:</strong> {hospital.phone || 'Not available'}</p>
                          <p><strong>📧 Email:</strong> {hospital.email || 'Not available'}</p>
                          <p><strong>🏥 Type:</strong> {hospital.type || 'Hospital'}</p>
                        </div>
                          ...
                          <div className="bed-stats">
                            {Object.entries(bedsByType).map(([type, stats]) => (
                              <div key={type} className="bed-stat">
                                <span className="bed-label">{getBedTypeIcon(type)} {type}:</span>
                                <span className="bed-value" style={{ color: stats.available > 0 ? '#10b981' : '#ef4444' }}>
                                  {stats.available}/{stats.total}
                                </span>
                              </div>
                            ))}
                            {bedsByType.GeneralWard && (
                              <div className="bed-stat">
                                <span className="bed-label">General Ward:</span>
                                <span className="bed-value">{bedsByType.GeneralWard.available}/{bedsByType.GeneralWard.total}</span>
                              </div>
                            )}
                          </div>
                          
                          {/* Detailed Bed List by Floor */}
                          {beds.length > 0 && (
                            <div className="bed-data-detailed">
                              <h5>Available Beds by Floor:</h5>
                              {beds.filter(bed => bed.status === 'available').slice(0, 5).map(bed => (
                                <div key={bed.id} className="ward-details">
                                  <strong>{getBedTypeIcon(bed.type)} {bed.type}</strong> - {bed.bedId}
                                  <span className="last-updated"> (Floor {bed.floor}, Room {bed.roomNumber})</span>
                                  <button 
                                    className="bed-request-btn-small"
                                    onClick={() => openBedRequestModal(hospital, bed)}
                        ...
                        <div className="hospital-footer">
                          <span className="established-year">Est. {hospital.year_established}</span>
                          {hospital.emergency_phone && (
                            <span className="emergency-contact">🚨 Emergency: {hospital.emergency_phone}</span>
                          )}
                          <button 
                  ...
                  <div className="search-placeholder">
                    <p>🔍 Enter a hospital name, city, or pincode to search</p>
                  </div>
                )}
              </div>
            </div>
      ...
      {showBedRequestModal && selectedHospitalForBed && (
        <div className="modal-overlay" onClick={() => setShowBedRequestModal(false)}>
          <div className="modal-content bed-request-modal" onClick={(e) => e.stopPropagation()}>
            <div className="modal-header">
              <h2>🛏️ Bed Request - {selectedHospitalForBed.name}</h2>
              <button className="modal-close" onClick={() => setShowBedRequestModal(false)}>✕</button>
            </div>
            <div className="modal-body">
              {bedRequestSuccess && (
              ...
              {selectedBedInfo && (
                <div className="selected-bed-info" style={{ background: '#e8f4fe', padding: '12px', borderRadius: '8px', marginBottom: '15px' }}>
                  <strong>Recommended Bed:</strong> {selectedBedInfo.bedId} ({selectedBedInfo.type}) - Floor {selectedBedInfo.floor}, Room {selectedBedInfo.roomNumber}
                </div>
              )}
              ...
              <form onSubmit={handleBedRequestSubmit}>
                ...
                <div className="form-actions">
                  <button type="button" className="cancel-btn" onClick={() => setShowBedRequestModal(false)}>
                    Cancel
                  </button>
                  <button type="submit" className="save-btn" disabled={submittingRequest}>
                    {submittingRequest ? 'Sending Request...' : 'Send Bed Request'}
                  </button>
                </div>
              </form>
            </div>
              ...
              <div className="stats-grid">
                ...
                <div className="stat-card">
                  ...
                  <div className="stat-details">
                    ...
                    <p className="stat-label">Pending: {bedRequests.filter(r => r.status === 'pending').length}</p>
                  </div>
                </div>
                <div className="stat-card">
                  <div className="stat-icon">📁</div>
                ...
                {bedRequests.slice(0, 3).map(request => (
                  <div key={request.id} className="activity-item">
                    ...
                    <div className="activity-details">
                      <p><strong>{request.hospitalName}</strong> - {request.requiredWard}</p>
                      <p className="activity-time">Requested: {new Date(request.createdAt).toLocaleDateString()}</p>
                      {request.priority && (
                        <p className="activity-time">Priority Score: {request.priority}</p>
                      )}
                    </div>
                    ...
                    <div key={request.id} className="bed-request-card">
                      ...
                      <div className="bed-request-details">
                        ...
                        <div className="detail-row">
                          ...
                          <span className="detail-value">{request.requiredWard}</span>
                        </div>
                        <div className="detail-row">
                          <span className="detail-label">Required Date:</span>
                          <span className="detail-value">{new Date(request.requiredDate).toLocaleDateString()}</span>
                        </div>
                        {request.priority && (
                          <div className="detail-row">
                            <span className="detail-label">AI Priority Score:</span>
                            <span className="detail-value" style={{ fontWeight: 'bold', color: request.priority > 70 ? '#dc2626' : request.priority > 40 ? '#f59e0b' : '#10b981' }}>
                              {request.priority}
                            </span>
                          </div>
                        )}
                        {request.recommendedBedType && (
                          <div className="detail-row">
                            <span className="detail-label">Recommended Bed:</span>
                            <span className="detail-value">{request.recommendedBedType}</span>
                          </div>
                        )}
                        {request.condition && (
                          <div className="detail-row">
                        ...
                        {request.medicalDescription && (
                          <div className="detail-row">
                            ...
                            <span className="detail-value">{request.medicalDescription}</span>
                          </div>
                        )}
                        {request.emergency && (
                          <div className="emergency-badge">🚨 Emergency Case</div>
                        )}
                        <div className="detail-row">
                      ...
                      {request.status === 'approved' && (
                        <div className="request-action">
                          <button className="contact-hospital-btn">📞 Contact Hospital</button>
                        </div>
                      )}
                    </div>
                  ))}
